...
function MinquiryDetail(props) {
   ...
   const minquiryListBtn = (event) => {
      event.preventDefault(); // 기본 동작 방지
      window.location.href = '/minquirylist'; // 클릭 시에만 이동
   };

   const minquiryUpdateBtn = (event) => {
      event.preventDefault(); // 기본 동작 방지
      window.location.href = '/minquiryupdate'; // 클릭 시에만 이동
   };
         ...
         <div className={board.board_container}>
            ...
            <div>               
               <div className={board.detail_button_box}>
                  <button className={board.cancle} onClick={minquiryListBtn}>목록</button>  
                  <form method="post">  
                     {/* <input type="hidden" name="cPage" value="${cPage}" /> 
                     <input type="hidden" name="idxn_idx" value="${gvo.idxn_idx}" /> 
                  ...
                  <span>
                     <button className={board.detail_write} onClick={minquiryUpdateBtn}>수정</button>   
