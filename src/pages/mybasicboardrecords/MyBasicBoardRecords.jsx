...
function MyBasicBoardRecords(props) {
   ...
   const basicRecordsDetailBtn = async (post_num) => {
      try {
         const response = await axios.post('http://localhost:8080/userrxtbl/detail', { post_num });
         // 응답에 따라 상세보기 페이지로 이동
         if (response.data && response.data.data && response.data.data.length > 0) {
            // 첫 번째 요소의 post_num을 사용
            const firstRecord = response.data.data[0];
            console.log("Navigating to detail with post_num:", firstRecord.post_num); // 이동할 post_num 출력
            navigate('/mybasicboardrecordsdetail', { state: { post_num: firstRecord.post_num } });
         } else {
            console.error("No records found in response:", response.data);
         }
      } catch (error) {
   ...
   const API_URL = `${process.env.REACT_APP_LOCAL_API_BASE_URL}/userrxtbl/list`;
         ...
         <section className={styles.mybasicboardrecords_container}>
            <div className={styles.records_grid}>
               {loading ? (
                  ...
                  list.map((item, index) => (
                     <div className={styles.records_item} key={item.id || index} onClick={() => basicRecordsDetailBtn(item.post_num)}>
                           <img
                              src={item.rx_photo}
                           ...
                           <div className={styles.records_text}>
                              <p>{item.rx_date}/{item.post_num}</p>
